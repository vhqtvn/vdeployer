 ...
 * @codeCoverageIgnore
 */
class WorkerSerializedCommand extends Command
{
    /**
     * @var Deployer
     */
    ...
    {
        parent::__construct('worker-serialized');
    ...
    {
        $defintions = $input->getArgument('serialized-definitions');
        $defintions = json_decode(hex2bin($defintions), true);
        $serializedArguments = $input->getArgument('serialized-arguments');
        $arguments = json_decode(hex2bin($serializedArguments), true);
        $new_definitions = $this->getDefinition();
        foreach ($arguments as $name => $value) {
            if(!$input->hasArgument($name)) {
                $new_definitions->addArgument(
                    new InputArgument(...$defintions[$name]),
                );
            }
        }
        $input->bind($new_definitions);
        foreach ($arguments as $name => $value) {
            if(!$input->hasArgument($name)) {
                $new_definitions->addArgument(
                    ...$defintions[$name],
